...
- **DESIGN CONTRACT SYNC:** If current approved UI code contradicts `./DESIGN.md`, do not blindly force the code back to a stale rule. First verify the implementation against existing patterns, then update `./DESIGN.md` and `changelog.md` so future UI work follows the validated source of truth.
