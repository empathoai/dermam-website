...
export default function TreatmentStepCard({ number, title, description }: TreatmentStepCardProps) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="relative flex h-full min-h-[9.5rem] flex-col bg-white p-8 rounded-[1rem] border border-border-card group hover:-translate-y-1 hover:border-border-card-hover hover:shadow-subtle transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
    >
