...
const ContactChannelCard: React.FC<ContactChannelCardProps> = ({ channel }) => {
 ...
 ? "bg-white border-base-900 shadow-card-soft shadow-lg hover:-translate-y-1 hover:shadow-subtle transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
 : "bg-white border-border-card hover:border-border-card-hover hover:shadow-subtle hover:-translate-y-1 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]";
 ...
 className={`flex items-center gap-5 p-6 rounded-[1rem] border group ${colors}`}
 >
 <div className={`w-12 h-12 rounded-[0.625rem] flex items-center justify-center flex-shrink-0 transition-colors ${channel.emphasized ? 'bg-base-900 text-base-100' : 'bg-canvas text-base-900 group-hover:bg-base-900/10'}`}>
 <Icon size={24} />
 </div>
