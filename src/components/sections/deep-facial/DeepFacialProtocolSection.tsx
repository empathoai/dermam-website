      ...
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {protocol.steps.map((step, idx) => (
            <div key={idx} className={`relative flex flex-col h-full bg-white rounded-2xl border transition-all duration-300 ${step.isKeyMoment ? 'border-sage ring-4 ring-sage-soft/30' : 'border-border-soft hover:border-sage'}`}>
              {step.isKeyMoment && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
                  <span className="bg-sage text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full shadow-lg">
                    Momento clave
                  </span>
                </div>
              )}
              ...
              <div className="p-6 flex flex-col flex-grow">
                <div className="flex items-center gap-3 mb-3">
                    <span className="text-2xl font-serif text-text-primary opacity-20">0{idx + 1}</span>
