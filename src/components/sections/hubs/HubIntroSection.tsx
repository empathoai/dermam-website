          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {content.stats.map((stat, idx) => (
              <motion.div 
                ...
                className="text-center p-8 bg-canvas/30 rounded-3xl border border-border-soft"
              >
                <div className="text-4xl md:text-5xl font-serif text-text-primary mb-2">
                  {stat.value}
                </div>
          ...
          <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
            {content.benefits.map((benefit, idx) => (
              <motion.div 
                ...
                viewport={{ once: true }}
                className="flex items-start gap-6 p-6 rounded-2xl bg-white border border-border-soft/50 hover:border-sage-soft transition-colors group"
              >
